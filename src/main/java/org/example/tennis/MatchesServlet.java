...
public class MatchesServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        filterName = request.getParameter("filter_by_player_name");

        if (filterName == null) {

            //фильтрация не сделается
            ...
            session.getTransaction().commit();         //сделаешь обработку ошибок если вернулся пустой список этот случай нужно предусмотреть а то npe будет
            ...
            PlayerEntity playerNeedToFind = session.createQuery("FROM PlayerEntity p where p.name = :name", PlayerEntity.class).setParameter("name", filterName).uniqueResult();


            List<MatchEntity> matches = session.createQuery("FROM MatchEntity m " +
                            "WHERE m.player1 = :player or m.player2 = :player " +
                            "ORDER BY m.id DESC",
                            MatchEntity.class)
                    .setParameter("player", playerNeedToFind)
                    .setFirstResult(fromIndex) // начиная с 6-й записи (индекс 5)
                    .setMaxResults(5)           // получить 5 записей
                    .getResultList();


            Long totalMatches = session.createQuery("SELECT COUNT(m) FROM MatchEntity m WHERE m.player1 = :player or m.player2 = :player", Long.class).setParameter("player", playerNeedToFind).getSingleResult();          //Зачем unique тут?
            totalPages = (int) Math.ceil((double) totalMatches / 5);
